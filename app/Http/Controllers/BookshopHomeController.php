...
class BookshopHomeController extends Controller
{
    ...
    {
        # Home page Books
        $vanhoc_books = Book::with('category')->whereHas('category', function($query) {
            $query->where('slug', 'vanhoc'); })
            ->take(4)
            ->latestFirst()
            ->get();
        $cntt_books = Book::with('category', 'author', 'image')
            ->whereHas('category', function ($query){
                $query->where('slug', 'cntt'); })
            ->take(4)
            ->latestFirst()
            ->get();
        ...
        return view('public.home', compact('vanhoc_books', 'discount_books', 'cntt_books'));
    }
    ...
    {
        # ComposerServiceProvider load here
        $discountTitle = "Tất cả sách giảm giá";
        $books = Book::with('author', 'image', 'category')
            ->orderBy('discount_rate', 'DESC')
            ->where('discount_rate', '>', 0)
            ->paginate(16);
