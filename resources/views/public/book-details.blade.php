                                ...
                                <div class="row">
                                    ...
                                    <div class="col-md-6">
                                        ...
                                        <div class="author mb-2">
                                            Của <a href="{{route('author', $book->author->slug)}}">{{$book->author->name}}</a>
                                        </div>
                                        @if(($book->quantity) > 1)
                                        ...
                                        @if($book->discount_rate)
                                            <h6><span class="badge badge-warning">{{$book->discount_rate}}% Giảm Giá</span></h6>
                                        @endif
                                        <div class="book-price mb-2">
                                            <span class="mr-1">Giá</span>
                                            @if($book->discount_rate)
                                                <span></span><strong class="line-through">{{$book->init_price}} VNĐ</strong>
                                            @endif
                                                <span>now</span><strong> {{$book->price}} VNĐ</strong>
                                            @if($book->discount_rate)
                                                <div><strong class="text-danger">Save {{$book->init_price - $book->price}} VNĐ</strong></div>
                                            @endif
                                        </div>
                                        <div class="book-category mb-2 py-1 d-flex flex-row border-top border-bottom">
                                            ...
                                            <a href="#review-section" class="mr-4"><i class="fas fa-comments"></i> Phản Hồi</a>
                                        </div>
