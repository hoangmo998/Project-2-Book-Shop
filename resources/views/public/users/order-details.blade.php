@extends('layouts.user-master')

@section('content')
<div class="container">
    <h4 class="my-4 p-4 bg-light">Chi Tiết Đặt Hàng</h4>

    <div class="card my-4">
        <div class="card-header">
            <h4>Thông Tin Khách Hàng</h4>
        </div>
        <div class="card-body">
            <p><i class="fas fa-user"></i> <span class="mx-2">{{$order->shipping->shipping_name}}</span></p>
            <p><i class="fas fa-phone"></i><span class="mx-2">{{$order->shipping->mobile_no}}</span></p>
            <p><i class="fas fa-map-marked"></i> <span class="mx-2">{{$order->shipping->address}}</span></p>
        </div>
    </div>
    <div class="order-product mb-4">
        <h4 class="my-4 p-4 bg-light">Thông Tin Đơn Hàng</h4>
        <table class="table">
        <thead>
        <tr>
            <th scope="col">Tên Sách</th>
            <th scope="col">Số Lượng</th>
            <th scope="col">Giá</th>
            <th scope="col">Tổng</th>
        </tr>
        </thead>
        <tbody>
        @foreach($order_details as $order_detail)
        <tr>
          <td>{{$order_detail->book_name}}</td>
          <td>{{$order_detail->book_quantity}}</td>
          <td>{{$order_detail->price}} VNĐ</td>
          <td>{{$order_detail->price * $order_detail->book_quantity}} VNĐ</td>
        </tr>
        @endforeach
        <tr>
            <td colspan="2"></td>
            <td><strong>Total</strong></td>
            <td><strong>{{$order->total_price}} VNĐ</strong></td>
        </tr>
        </tbody>
        </table>
    </div>
</div>
@endsection
